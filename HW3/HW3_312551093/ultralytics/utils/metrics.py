...
# Center coordinates calculation remains the same
...
# Center distance
    center_distance = ((b2_center_x - b1_center_x[:, None])**2 + (b2_center_y - b1_center_y[:, None])**2)

# Diagonal distance of the smallest enclosing box
    enclosing_x1 = np.minimum(b1_x1[:, None], b2_x1)
    enclosing_y1 = np.minimum(b1_y1[:, None], b2_y1)
    enclosing_x2 = np.maximum(b1_x2[:, None], b2_x2)
    enclosing_y2 = np.maximum(b1_y2[:, None], b2_y2)
    diagonal_distance = (enclosing_x2 - enclosing_x1)**2 + (enclosing_y2 - enclosing_y1)**2

# Aspect ratio
    aspect_ratio_b1 = (b1_x2 - b1_x1) / np.maximum((b1_y2 - b1_y1), eps)
    aspect_ratio_b2 = (b2_x2 - b2_x1) / np.maximum((b2_y2 - b2_y1), eps)
    aspect_ratio_term = 4 / (np.pi**2) * (np.arctan(aspect_ratio_b2) - np.arctan(aspect_ratio_b1[:, None]))**2

# CIoU
    ciou = iou - (center_distance / (diagonal_distance + eps)) - aspect_ratio_term
